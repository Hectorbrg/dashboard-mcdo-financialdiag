...
def line_chart(years, series_dict, y_format="$,.0f", height=320):
    ...
    for i, (label, values) in enumerate(series_dict.items()):
        ...
        # Drop None pairs so the line doesn't crash
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate="<b>%{x}</b><br>" + label + ": %{y:" + y_format + "}<extra></extra>",
        ))
...
def bar_chart(years, series_dict, y_format="$,.0f", height=320, mode="group"):
    ...
    for i, (label, values) in enumerate(series_dict.items()):
        ...
        fig.add_trace(go.Bar(
            x=years, y=_safe(values), name=label,
...
def donut_chart(labels, values, height=300, colors=None):
    ...
    fig.update_layout(**layout)
    # Centre annotation showing the total
    fig.add_annotation(
        text=f"<b>${total/1000:,.1f}B</b><br><span style='font-size:10px;color:{SUBTLE}'>TOTAL</span>",
...
def heatmap_matrix(years, ratio_dict, height=460):
    ...
    layout["xaxis"] = dict(side="top", color=YELLOW,
